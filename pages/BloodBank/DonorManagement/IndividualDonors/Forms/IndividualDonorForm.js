...
//@mui
import { Box, Button, Card, Grid, MenuItem, Step, StepLabel, Stepper, ToggleButton, Typography } from '@mui/material';
...
const Schema = yup.object().shape({
  ...
  relationName: yup.string().trim().required('Required'),
  ...
  collectionDate: yup.date('Invalid').nullable().required('Required'),
...
export default function IndividualDonorForm({ onClose, getFormResponse, campId }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: ({ data }) => {
      // handledialog();
      if (campId) {
        queryClient.invalidateQueries({ queryKey: ['getAllCampDonorList', campId] });
      } else {
        queryClient.invalidateQueries({ queryKey: ['getAllIndividualDonorList'] });
      }
      toast(data || saveMessage);
      data && getFormResponse(data);
      onClose();
    },
    onError: (error) => {
  ...
  const onSubmit = (data) => {
    var request = {
      ...data,
      donorRegType: campId ? campId : null,
  ...
  const steps = ['Individual Donor', 'Physical Examination'];
    ...
    <FormWrapper
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
          ...
          <Grid container justifyContent='center'>
            ...
            <Grid item>
              {activeStep === steps.length - 1 ?
                ...
                :
                <Button onClick={handleNext} variant='contained' color='inherit'>
                  Next
                </Button>
              }
            </Grid>
        ...
        {/* <DialogBox open={dialogOpen}>
          <Saviour dialogProperty={handleDialogClose} />
        </DialogBox> */}
      </FormProvider >
    </FormWrapper >
  );
