    ...
    <BrowserRouter>
      <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={In}>
        <QueryClientProvider client={queryClient}>
          <FuelDetails onClose={() => {}} isEditMode={false} row={null} fuelDetails={null} />
        </QueryClientProvider>
...
jest.mock('pages/api/transport', () => ({
  fuelTypeGetAllFuelType: jest.fn(),
  fuelTypeSaveFuelType: jest.fn(),
}));

describe('FuelType Component', () => {
  beforeEach(() => jest.clearAllMocks());

  it('should render Fuel Types when API responds', async () => {
    ...
    api.fuelTypeGetAllFuelType.mockImplementation(async (options) => {
    ...
    await waitFor(() => {
      expect(api.fuelTypeGetAllFuelType).toHaveBeenCalledWith(
...
describe('AddFuelType Component', () => {
  test('handles form submission', async () => {
    ...
    const FuelTypeInput = getByTestId('Fuel-type-input');

    fireEvent.change(FuelTypeInput, { target: { value: 'Test Fuel' } });

    api.fuelTypeSaveFuelType.mockImplementation(async (options) => {
      // Check if the signal property is an AbortSignal instance
      if (options.signal instanceof AbortSignal) {
        // Handle the signal object if needed
      }
