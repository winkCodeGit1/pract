...
import BatteryDetails from 'pages/Transport/batteryType/BatteryDetails';
// import { Api } from '@mui/icons-material';
import AddBattery from '../batteryType/index';
// import * as api from 'pages/api/transport';
...
jest.mock('pages/api/transport', () => ({
  getBatteryType: jest.fn(),
  saveBatteryType: jest.fn(),
}));

describe('BatteryType Component', () => {
  beforeEach(() => jest.clearAllMocks());

  it('should render Battery Types when API responds', async () => {
    // For this test, when labAll is called, return the given value wrapped in a Promise

    // const responseData = [{ type: 'Test123' }];
    // Mock the API function to resolve with the response data
    api.getBatteryType.mockImplementation(async (options) => {
    ...
    await waitFor(() => {
      expect(api.getBatteryType).toHaveBeenCalledWith(
...
describe('AddBatteryType Component', () => {
  test('handles form submission', async () => {
    const { getByTestId, getByText } = render(
      <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={In}>
        <QueryClientProvider client={queryClient}>
          <BatteryDetails onClose={() => {}} selectedRow={null} isEditMode={false} />
        </QueryClientProvider>
      </LocalizationProvider>
    );
    const BatteryTypeInput = getByTestId('Battery-type-input');

    fireEvent.change(BatteryTypeInput, { target: { value: 'Test Battery' } });

    api.saveBatteryType.mockImplementation(async (options) => {
      // Check if the signal property is an AbortSignal instance
      if (options.signal instanceof AbortSignal) {
        // Handle the signal object if needed
      }
